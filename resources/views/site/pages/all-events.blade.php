<x-layouts.frontend>
    {{-- @dd($events); --}}
    @section('content')
        <div class="pb-20 bg-scroll min-h-screen" style="background-image: url('{{ asset('site/images/events.png') }}">
            <div
                    ...
                    <ul class="flex flex-wrap -mb-px text-lg font-medium text-center events xmd:space-y-0 md:space-y-2 sm:space-y-2 xxsm:space-y-2"
                        ...
                        @foreach ($events as $category)
                            <li class="mr-2">
                                <button
                                    class="inline-block p-4 px-6 py-3 rounded-md z-20 font-poppins shadow-md focus:outline-none relative"
                                    id="category_{{ $category->id }}" data-tabs-target="#category_{{ $category->id }}"
                                    type="button" role="tab" aria-controls="category_{{ $category->id }}"
                                    aria-selected="false">
                                    {{ $category->name }}
                                </button>
                            </li>
                        ...
                        <li class="mr-2">
                            <button
                                ...
                                aria-controls="online" aria-selected="false">Online Events(4)</button>
                        </li>
                        <li class="mr-2">
                            <button
                                class="inline-block z-20 px-5 py-3 rounded-md font-poppins shadow-md focus:outline-none relative"
                                id="venue_events" data-tabs-target="#venue" type="button" role="tab"
                                aria-controls="venue" aria-selected="false">Venue Events(4)</button>
                        </li>
                    </ul>
                </div>
                <div id="myTabContent">
                                ...
                                @foreach ($events as $item)
                                    <div
                                        ...
                                        <div class="flex justify-between mt-7">
                                            <a href="{{ url('event-detail/' . $item['id'] ?? '') }}"
                    ...
                    <div class="hidden" id="online" role="tabpanel" aria-labelledby="online_events">
                        <div
                            ...
                            @foreach ($onlineEvents as $online)
                                <div class="shadow-2xl p-5 rounded-lg bg-white hover:scale-110 transition-all duration-500">
                                    <a
                                        href="{{ url('event/' . $online->id . '/' . \Illuminate\Support\Str::slug($online->title)) }}">
                                        <img src="{{ asset('uploads/events/' . $online->image) }}" alt=""
                                            class="h-40 rounded-lg w-full object-cover bg-cover">
                                        <p class="font-popping font-semibold text-xl leading-8 pt-2">{{ $online->title }}
                                        </p>
                                        <p class="font-poppins font-normal text-base leading-6 text-gray pt-1">
                                            {{ \Carbon\Carbon::parse($online->start_date)->format('d M Y') }} -
                                            {{ \Carbon\Carbon::parse($online->end_date)->format('d M Y') }}
                                        </p>
                                    </a>
                                    <div class="flex justify-between mt-7">
                                        <a href="{{ url('event-detail/' . $item['id'] ?? '') }}"
